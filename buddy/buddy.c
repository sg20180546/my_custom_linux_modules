...
//|real_memory
// struct page  // bitmap // physical page //
...
void init_memory(void){
    ...
    mem_offset=sizeof(struct page)*TOTAL_PAGES(mem_size); // to use at get_address_map
...
struct page* __alloc_pages(unsigned int gfp_mask,unsigned int order,zonelist_t *zonelist){
    ...
    do{
        head=&area->free_list;
        curr=head->next;
        if(curr!=head){ // if there is page in low level
            unsigned long index;
